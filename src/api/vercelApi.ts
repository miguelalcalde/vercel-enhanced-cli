...
export class VercelApi {
  ...
  async listTeams(): Promise<VercelTeam[]> {
    const response =
      await this.request<PaginatedResponse<VercelTeam>>("/v2/teams");
  ...
  async listProjects(teamId?: string | null): Promise<VercelProject[]> {
    ...
    do {
      ...
      const response =
        await this.request<PaginatedResponse<VercelProject>>(endpoint);
  ...
  async listDeployments(
    ...
    do {
      ...
      const response =
        await this.request<PaginatedResponse<VercelDeployment>>(endpoint);
