...
export async function showProjectDetails(
  ...
  try {
    ...
    if (projectDetails.link) {
      ...
      const repoName =
        projectDetails.link.repoName ||
        (projectDetails.link.repo
          ? projectDetails.link.repo.split("/").pop()
          : null) ||
  ...
  return new Promise((resolve, reject) => {
    ...
    const render = () => {
      ...
      if (domains.length > 0) {
        domains.forEach((domain, index) => {
          ...
          const verifiedIcon = domain.verified
            ? chalk.green("✓")
            : chalk.yellow("⚠");
          const verifiedText = domain.verified
            ? ""
          ...
          console.log(
            `${prefix}${verifiedIcon} ${domainName}${redirectInfo}${verifiedText} ${chalk.gray(`(${typeBadge})`)}`,
          );
        });
      } else {
        ...
        if (org && repoName) {
          ...
          const provider =
            projectDetails.link.type === "github"
              ? "GitHub"
              : projectDetails.link.type === "gitlab"
                ? "GitLab"
                : projectDetails.link.type;
          ...
          console.log(
            `${prefix}${repoNameDisplay} ${chalk.gray(`(${branch} branch)`)}`,
          );
      ...
      const framework = projectDetails.framework
        ? projectDetails.framework.charAt(0).toUpperCase() +
          projectDetails.framework.slice(1)
        : chalk.gray("Auto-detected");
      const buildCommand =
        projectDetails.buildCommand || chalk.gray("Auto-detected");
      const devCommand =
        projectDetails.devCommand || chalk.gray("Auto-detected");
      const installCommand =
        projectDetails.installCommand || chalk.gray("Auto-detected");
      const outputDirectory =
        projectDetails.outputDirectory || chalk.gray("Default");
      const rootDirectory =
        projectDetails.rootDirectory || chalk.gray("Repository root");
    ...
    const handleData = (chunk: Buffer) => {
      ...
      if (
        ...
        cursorIndex < selectableItems.length
      ) {
      ...
      if (
        ...
        import("clipboardy")
          ...
          .then(() => {
            console.log(
              chalk.green(`✓ Copied "${selectedItem.url}" to clipboard`),
            );
            setTimeout(() => {
              showInteractiveProjectDetails(
                project,
          ...
          .catch((error) => {
            console.log(
              chalk.yellow(
                `⚠ Clipboard not available. URL: ${chalk.bold(selectedItem.url)}`,
              ),
            );
            setTimeout(() => {
              showInteractiveProjectDetails(
